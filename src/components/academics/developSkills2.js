...
const categories = [
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setActiveSubcategory(0); // Reset subcategory index when the category changes
    }, 2000); // Change category every 2 seconds
    return () => clearInterval(interval); // Cleanup interval on unmount
  }, []);

  // Change subcategory within the current category every 2 seconds
  useEffect(() => {
    ...
    }, 2000); // Change subcategory every 2 seconds
    return () => clearInterval(subcategoryInterval); // Cleanup interval on unmount
  }, [activeCategory]);
  ...
  const extraSmallScreen=useMediaQuery(theme.breakpoints.down("348"))
    ...
    onUpdate={({ x }) => {
        // When x is back to 0, it means the animation has returned to its original position
        if (x === 0) {
            setStart(false);
        }
        else if(x === -300){
